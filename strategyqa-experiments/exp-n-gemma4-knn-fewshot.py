...
# =============================================================================
# EXPERIMENT N (Gemma) — kNN dynamic few-shot (per-question semantic retrieval)
# Same pipeline as exp-n-knn-fewshot.py, but on gemma4:e4b for cross-model
...
TRAIN_SIZE     = 200      # pool of demo candidates
K              = 4        # demos per question
LIMIT          = 200      # number of EVAL items (taken from after the demo pool)
...
def run_benchmark():
    ...
    eval_items = data[TRAIN_SIZE:TRAIN_SIZE + LIMIT]
    ...
    for item in eval_items:
        ...
        print(f"  Answer: {raw_answer}  ({'OK' if is_correct else 'X'})  ({duration}s)  "
    ...
    output_file     = f"results_exp_n_knn_fewshot_gemma4_{MODEL_NAME}_{timestamp}.json"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            "metadata": {
                "model":           MODEL_NAME,
                "experiment":      f"N (Gemma) - kNN dynamic few-shot (K={K}, pool={len(train_pool)})",
