...
# =============================================================================
# EXPERIMENT O (Gemma) — Self-Refine / Reflexion (3-turn self-critique)
# Same pipeline as exp-o-self-refine.py, run on gemma4:e4b for cross-model
# comparison with the Qwen run (which showed a -5pp regression).
# =============================================================================
...
def run_benchmark():
    ...
    for item in tasks:
        ...
        print(f"  Initial: {initial_answer}  ->  Revised: {revised_answer}  "
              f"({'FLIPPED' if flipped else 'kept'})  ({'OK' if is_correct else 'X'})  ({duration}s)  "
    ...
    output_file     = f"results_exp_o_self_refine_gemma4_{MODEL_NAME}_{timestamp}.json"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            "metadata": {
                "model":           MODEL_NAME,
                "experiment":      "O (Gemma) - Self-Refine (Answer -> Critique -> Revise)",
                "total":           len(results),
