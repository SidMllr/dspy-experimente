...
# =============================================================================
# EXPERIMENT J — MIPROv2 on F-Ultimate signatures
#
# Direct optimizer comparison vs Bootstrap-only runs (qwen3_bootstrap.json).
# MIPROv2 jointly optimises *instructions* AND *demonstrations*, while
# BootstrapFewShot only fills in demos. If F-Ultimate already has hand-tuned
# instructions, MIPROv2 might be able to refine them further by exploring
# alternative phrasings the model itself proposes.
#
# What this does:
#   - Take the F-Ultimate signatures (DecomposeQuestion, ReasonAboutQuestion,
#     ConcludeFromReasoning) wrapped as a single dspy.Module.
#   - Use MIPROv2 (auto="light") with a Yes/No exact-match metric.
#   - Train on ~30 examples, validate on ~50, evaluate on the rest of LIMIT.
#   - Save the optimized program state and the answers it produces.
#
# Single variable changed vs F-Ultimate: optimizer = MIPROv2 instead of none.
# =============================================================================
...
# ---------------------------------------------------------------------------
# Signatures (identical to F-Ultimate so the optimizer comparison is fair)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Module wrapping the full pipeline (single SC sample inside the module so
# MIPROv2 can score deterministically; SC majority happens at evaluation time).
# ---------------------------------------------------------------------------
...
def main():
    ...
    state_file = f"exp_j_mipro_ultimate_state_{timestamp}.json"
    try:
    ...
    for item in eval_items:
        ...
        print(f"  Final: {final_answer}  ({'✅' if is_correct else '❌'})  "
    ...
    output_file     = f"results_exp_j_mipro_ultimate_{MODEL_NAME}_{timestamp}.json"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            "metadata": {
                "model":           MODEL_NAME,
                "experiment":      "J - MIPROv2 on F-Ultimate (joint instr+demos)",
