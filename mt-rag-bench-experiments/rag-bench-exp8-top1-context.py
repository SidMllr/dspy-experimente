...
# =============================================================================
# EXPERIMENT 8 — Top-1 context passage only  (vs. top-5 in baseline)
#
# Key finding driving this: hallucination is the dominant failure mode.
# With top-5 passages, irrelevant passages likely introduce noise and
# tempt the model to confabulate details. Using only the single most
# relevant passage forces it to work with focused, high-signal context.
#
# Single variable changed: raw_contexts[:1]  (baseline uses all 5)
...
async def run_benchmark_async():
    ...
    for index, item in enumerate(tasks):
        ...
        if isinstance(raw_contexts, list):
            # Use only the single most relevant passage
            top1 = raw_contexts[:1]
            context = "\n\n".join(
    ...
    output_file = f"qwen_rag_answers_exp8_top1_context_{timestamp}.json"
